...
@ADMIN.route('/admin/photos/verify')
@login.login_required
@login_manager.admin_required
def verify_photos():
    ...
    return flask.render_template(
        'admin/verify_photos.html',
        photo=photo,
        random=util.generate_key(5)
    )

@ADMIN.route('/admin/photo/<int:photo_id>/reject')
@login.login_required
@login_manager.admin_required
def reject_photo(photo_id):
    ...
    return flask.redirect(flask.request.referrer or
                          flask.url_for('admin.verify_photos'))

@ADMIN.route('/admin/photo/<int:photo_id>/accept')
@login.login_required
@login_manager.admin_required
...
@ADMIN.route('/admin/photo/<int:photo_id>/rotate/<int:degrees>')
@login.login_required
@login_manager.admin_required
